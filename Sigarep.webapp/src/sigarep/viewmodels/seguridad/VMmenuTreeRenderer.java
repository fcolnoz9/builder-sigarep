...
public class VMmenuTreeRenderer implements TreeitemRenderer<VMmenuTreeNode> {
	@Override
	public void render(final Treeitem treeItem, VMmenuTreeNode treeNode,int index) throws Exception {
		...
		dataRow.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			...
			private VMmenuTreeNode obtenePadres(VMmenuTreeNode padre,
					VMmenuTreeNode hijo) {
			...
			private void agregarNodo(VMmenuTreeNode nodo,
					VMmenuTreeNode root) {
				boolean encontro = false;
				for (int j = 0; j < root.getChildCount(); j++) {
					if (root.getChildAt(j).getData() == nodo.getData()) {
						agregarNodo((VMmenuTreeNode) nodo.getChildAt(0),
								(VMmenuTreeNode) root.getChildAt(j));
						encontro = true;
						break;
					}
				}
