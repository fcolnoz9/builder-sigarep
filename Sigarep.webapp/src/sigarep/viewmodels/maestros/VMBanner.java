...
public class VMBanner {
	...
	@Command
	// Permite manipular la propiedad de ViewModel
	@NotifyChange({ "descripcion", "enlace", "fechaVencimiento", "titulo",
			"estatus", "imagenBanner", "listadoBanner" })
	// el notifychange le avisa a que parametros en la pantalla se van a
	// cambiar, en este caso es los atributos de la pantalla se va a colocar en
	// blanco al guardar!!
	public void guardarBanner() {
		if (descripcion == null || enlace == null || titulo == null)
			...
			Banner ban = new Banner(idImagen, descripcion, enlace,
					fechaVencimiento, titulo, fotoBanner, true);
	...
	@NotifyChange({ "listadoBanner", "descripcion", "fechaVencimiento",
			"enlace", "titulo", "imagenBanner" })
	public void eliminarImagenBanner(
			@ContextParam(ContextType.BINDER) final Binder binder) {
		if (titulo == null || enlace == null || descripcion == null
				|| fechaVencimiento == null || fotoBanner.getTamano() < 1) {
	...
	public void mostrarSeleccionado() {
		...
		if (bannerSeleccionado.getFotoBanner().getTamano() > 0) {
			try {
				imagenBanner = new AImage(bannerSeleccionado.getFotoBanner()
						.getNombreArchivo(), bannerSeleccionado.getFotoBanner()
						.getContenidoArchivo());
			} catch (IOException e) {
	...
	public void cargarImagenBanner(
	...
	public void filtrosBanner() {
		listadoBanner = servicioBanner.buscarFiltroBanner(tituloFiltro,
				enlaceFiltro);
	}
