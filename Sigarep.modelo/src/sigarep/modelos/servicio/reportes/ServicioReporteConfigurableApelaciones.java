...
public class ServicioReporteConfigurableApelaciones {
	...
	public List<ConfigurableApelaciones> buscarTodasApelaciones(String lapsoAcademico,String tiposancion,String instanciaApelada,String tipoMotivo, String programaAcademico,String veredicto,String edoApelacion) {
		...
		"FROM sigarep.estado_apelacion edo_ape, sigarep.estudiante es  " +
		"INNER JOIN sigarep.estudiante_sancionado as esa ON esa.cedula_estudiante=es.cedula_estudiante " +
	    "INNER JOIN sigarep.solicitud_apelacion as sap ON sap.cedula_estudiante=esa.cedula_estudiante " +
	    "INNER JOIN sigarep.lapso_academico as lapso ON lapso.codigo_lapso=sap.codigo_lapso " +
	    "INNER JOIN sigarep.motivo as mot ON mot.cedula_estudiante=sap.cedula_estudiante " +
	    "INNER JOIN sigarep.tipo_motivo as tm ON tm.id_tipo_motivo=mot.id_tipo_motivo " +
	    "INNER JOIN sigarep.instancia_apelada as iap ON iap.id_instancia_apelada=sap.id_instancia_apelada " +
	    "INNER JOIN sigarep.programa_academico as prog ON prog.id_programa=es.id_programa " +
	    "INNER JOIN sigarep.sancion_maestro as san ON san.id_sancion=esa.id_sancion " +
	    "INNER JOIN sigarep.apelacion_estado_apelacion as ap_edo ON ap_edo.cedula_estudiante =sap.cedula_estudiante " +
